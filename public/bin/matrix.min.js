...
Sentence.prototype.createSentence=function(a,b=5,d){let c=State.getInstance();const e=this;for(let f=0;f<a.length+1;f++){let h=f===a.length;setTimeout(()=>{let g=new Char(350+20*b,h?Math.floor(100*Math.random()):null,h?null:a[f]);g.x=Char.SIZE/2*f;g.y=-(Char.SIZE+5);e.addChild(g);e.allChars.push(g);createjs.Sound.play("frap_"+Math.ceil(4*Math.random()));f>this.limitQuote&&!h&&g.color(c.currentColor)},50*f)}};Sentence.prototype.update=function(){this.allChars.forEach(a=>{a.update()})};
...
Matrix.prototype.update=function(){const a=this;this.sentence&&this.sentence.update();this.command&&this.command.update();for(let b=0;b<this.rain.length;b++)this.rain[b].update();this.colorCount++;500<=this.colorCount&&(this.colorCount=0,this.state.isRandom||this.state.getNewColor());this.glitchCount++;this.glitchCount>=this.glitchTrigger&&(this.glitchTrigger=Math.floor(350*Math.random())+100,this.glitchCount=0,this.updateGlitch(),this.glitchFilter.enabled=!0,setTimeout(()=>{a.glitchFilter.enabled=
